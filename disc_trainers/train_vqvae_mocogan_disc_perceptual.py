# This code uses the mocogan content and temporal discriminators with perceptual loss
...
from utils import *
from config import DATASET, LATENT_LOSS_WEIGHT, PERCEPTUAL_LOSS_WEIGHT, image_disc_weight, video_disc_weight, SAMPLE_SIZE_FOR_VISUALIZATION
...
def base_validation(model, vqlpips, val_loader, device, epoch, i, run_type, sample_folder):
    ...
    for val_i, data in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            sample, _, out, source_img = run_step(model, vqlpips, data, device, 'val')
...
def train_generator(opt, image_discriminator, video_discriminator, 
                    fake_batch, recon_loss, latent_loss, perceptual_loss):
    opt.zero_grad()
...
def train(model, patch_image_disc, patch_video_disc, 
        ...
        epoch, validate_at, checkpoint_dir, sample_folder, vqlpips):
    ...
    for i, data in enumerate(loader):
        ...
        recon_loss, latent_loss, S, out, ground_truth, perceptual_loss = run_step(model, vqlpips, data, device)
        ...
        gen_loss = train_generator(gen_optim, patch_image_disc, patch_video_disc, fake_sampled, recon_loss, latent_loss, perceptual_loss)
...
def main(args):
    ...
    train_loader, val_loader, model, patch_image_disc, patch_video_disc, \
        image_disc_optim, video_disc_optim, vqlpips = \
    ...
    if args.test:
        ...
        for i in range(args.epoch):
            ...
            train(model, patch_image_disc, patch_video_disc, 
                ...
                args.validate_at, args.checkpoint_dir, args.sample_folder, vqlpips)
...
if __name__ == "__main__":
    ...
    print(f'Weight configuration used, latent loss : {LATENT_LOSS_WEIGHT}, \
