# This is the code with the perceptual loss and the MoCoGAN-HD discriminator
# Currently the weight corresponding to the perceptual loss is also set to 1
...
from utils import *
from config import DATASET, LATENT_LOSS_WEIGHT, G_LOSS_2D_WEIGHT, G_LOSS_3D_WEIGHT, D_LOSS_WEIGHT, PERCEPTUAL_LOSS_WEIGHT, SAMPLE_SIZE_FOR_VISUALIZATION
...
def run_step(model, data, device, vqlpips, run='train'):
    ...
    perceptual_loss = vqlpips(out, ground_truth)
    
    if run == 'train':
        return recon_loss, latent_loss, S, out, ground_truth, perceptual_loss
    else:
...
def jitter_validation(model, val_loader, device, epoch, i, run_type, sample_folder, vqlpips):
    for i, data in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            source_images, input, prediction, source_images_original = run_step(model, data, device, vqlpips, run='val')
...
def base_validation(model, val_loader, device, epoch, i, run_type, sample_folder, vqlpips):
    ...
    for val_i, data in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            sample, _, out, source_img = run_step(model, data, device, vqlpips, 'val')
...
def validation(model, val_loader, device, epoch, i, sample_folder, vqlpips, run_type='train'):
    if dataset >= 6:
        jitter_validation(model, val_loader, device, epoch, i, run_type, sample_folder, vqlpips)
    else:
        base_validation(model, val_loader, device, epoch, i, run_type, sample_folder, vqlpips)
...
def train(model, loader, val_loader, scheduler, device, 
            ...
            modelD_img, modelD_3d, gen_optim, vqlpips):

    SAMPLE_FRAMES = 12 # sample 16 frames for the discriminator

    global global_step

    for i, data in enumerate(loader):
        recon_loss, latent_loss, S, out, ground_truth, perceptual_loss = run_step(model, data, device, vqlpips)
        ...
        # the generator can be optimized for a few more steps than the discriminator
        if i%2 == 0:
            step = 'gen'
        else:
            step = 'disc'
        ...
        if i%validate_at == 0:
            ...
            validation(model, val_loader, device, epoch, i, sample_folder, vqlpips)
...
def main(args):
    ...
    train_loader, val_loader, model, modelD_img, modelD_3d, vqlpips = \
    ...
    if args.ckpt:
        print(f'Loading pretrained model : {args.ckpt}')
    ...
    if args.test:
        validation(model, val_loader, device, 0, 0, args.sample_folder, vqlpips, 'val')
    else:
        ...
        for i in range(args.epoch):
            ...
            train(model, train_loader, val_loader, scheduler, device, i, 
                ...
                modelD_img, modelD_3d, optimizer, vqlpips)
...
if __name__ == "__main__":
    ...
    print(f'Loss weight configurations being used : {PERCEPTUAL_LOSS_WEIGHT}, \
        2d gen weight {G_LOSS_2D_WEIGHT}, 3d gen weight : {G_LOSS_3D_WEIGHT}')
