'''
For generating the masked region, use the bounding box around the landmarks
'''
...
'''
resize the frame
the learned transformation is a combination of affine, non-linear, and color transformations
'''
...
'''
generate the convex hull based on the coordinates of the bounding box
'''
...
'''
generates the convex hull mask of the face
'''
...
'''
Enlarge the face mask to accomodate the region close to the face boundary
'''
...
'''
Heuristic blending of the face images using poisson blend
'''
...
'''
Combines the segmented face image (foreground) with the segmented face mask (background)
'''
...
'''
Computes the rotation of the face w.r.t the horizontal 
The angle of the line connecting the eye centroids is estimated 
'''
...
'''
Apply the mask on the input face image
'''
def apply_mask(mask, image):
    return ((mask / 255.) * image).astype(np.uint8)

'''
Estimate the coordinates of the bounding box from the face landmarks
'''
