# This is the mocoganhd content discriminator code
import numpy as np
import torch
import torch.nn as nn
import functools

# this is the image discriminator - for classifying the content as real/fake 
class ModelD_img(nn.Module):
    def __init__(self, nc, norm_D_3d, num_D, lr):
...
class MultiscaleDiscriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(num_D):
            ...
            # the model needs to be transferred to the GPU first
            result.append(self.singleD_forward(model, input_downsampled)) # the model is not on the GPU, that's why the error is being thrown
            if i != (num_D - 1):
